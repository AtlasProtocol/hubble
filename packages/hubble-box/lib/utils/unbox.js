  ...
  return Promise.resolve().then(function() {
    ...
    if (contents.length) {
      var err = "Something already exists at the destination. " +
                "`hubble init` and `hubble unbox` must be executed in an empty folder. " +
                "Stopping to prevent overwriting data."

      throw new HubbleError(err);
    }
  })
}
  ...
  return new Promise(function(accept, reject) {
    ...
    request(options, function(error, r) {
      if (error) {
        ...
        return reject(new Error("hubble Box at URL " + url + " doesn't exist. If you believe this is an error, please contact hubble support."));
...
function readBoxConfig(destination) {
  var possibleConfigs = [
    path.join(destination, "hubble-box.json"),
    path.join(destination, "hubble-init.json")
  ];
...
function cleanupUnpack(boxConfig, destination) {
  ...
  needingRemoval.push("hubble-box.json");
  needingRemoval.push("hubble-init.json");
